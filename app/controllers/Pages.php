...
class Pages extends Controller {
    ...
    public function index() {
        ...
        if($pages_category_url) {
            ...
            $pages_category = $pages_category_url ? database()->query("SELECT * FROM `pages_categories` WHERE (`url` = '{$pages_category_url}' AND `language` = '{$language}') || (`url` = '{$pages_category_url}' AND `language` IS NULL)")->fetch_object() ?? null : null;
            ...
            $pages_result = database()->query("SELECT `url`, `title`, `description`, `total_views`, `type` FROM `pages` WHERE `pages_category_id` = {$pages_category->pages_category_id} AND (`language` = '{$language}' OR `language` IS NULL) ORDER BY `total_views` DESC");
            ...
            $popular_pages_result = database()->query("SELECT `url`, `title`, `description`, `total_views`, `type` FROM `pages` WHERE (`language` = '{$language}' OR `language` IS NULL) ORDER BY `total_views` DESC LIMIT 6");
